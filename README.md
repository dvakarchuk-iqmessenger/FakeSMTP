[IQM: How to test email with your own encrypted SMTP server (to check self-signed certs)](https://iqmessenger.atlassian.net/wiki/spaces/IQM/pages/382349082625/How+to+test+email+with+your+own+encrypted+SMTP+server+to+check+self-signed+certs)
...
The `fakeSMTP.jar` file is executable.  
If your desktop environment supports it, you can simply double-click the `.jar` file.

Otherwise, run:
```bash
java -jar fakeSMTP-VERSION.jar
```
After the UI opens, click the Start server button.
This starts a <b>TLS</b>-encrypted SMTP server using a self-signed certificate.

### Starting in SSL Mode
To start the server in <b>SSL</b> mode:
```bash
java -jar fakeSMTP-2.1-SNAPSHOT.jar SSL
```
### Using a Custom Certificate

To use your own server certificate, pass the absolute path to the keystore and its password
(both the keystore password and the key password must be the same):
```bash
java -jar fakeSMTP-2.1-SNAPSHOT.jar TLS C:\Users\user\keystore.jks password
```
To use the same custom certificate in SSL mode:
```bash
java -jar fakeSMTP-2.1-SNAPSHOT.jar SSL C:\Users\user\keystore.jks password
```
